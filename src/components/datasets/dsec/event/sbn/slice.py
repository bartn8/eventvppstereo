...
class EventSlicer(torch.utils.data.Dataset):
    def __init__(self, event_root, rectify_map_root, num_of_event, num_of_future_event=0):
        ...
        try:
            ...
            raise OSError(f"{str(e)}; filename: {event_root} or {rectify_map_root}") from e
    ...
    def __getitem__(self, ts_end):
        try:
            ...
            raise OSError(f"{str(e)}; filename: {self.event_root} or {self.rectify_map_root}") from e
    ...
    def rectify_events(self, event_data):
        try:
            ...
            raise IndexError(f"{str(e)}; filename: {self.rectify_map_root}") from e
        x_rect = xy_rect[:, 0]
        y_rect = xy_rect[:, 1]
