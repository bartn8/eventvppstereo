...
class EventDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, mydict):
        ...
        if self.use_preprocessed_image and hints is None:
            ...
            try:
                ...
                raise IndexError(f"{str(e)}; filename={self.root}") from e
            except AssertionError as e:
                raise AssertionError(f"{str(e)}; filename={self.root}") from e
    ...
    def _pre_load_event_data(self, timestamp, hints, guide_args):
        ...
        guide_method = guide_args['guide_method'] if guide_args is not None else 'none'
        ...
        if guide_args['render']:
            ...
            img_left = render(img_left, left_history[0], left_history[1], left_history[2], left_history[3], scale_factor=scale_factor)
            img_right = render(img_right, right_history[0], right_history[1], right_history[2], right_history[3], scale_factor=scale_factor)
            ...
            for key in rendered_data:    
                if n_images > 1:                
                    ...
                    cv2.imwrite(os.path.join(guide_args['savedir'], f'{self.seq_name}_{self.mycounter:05d}_{key}.jpg'), cv2.cvtColor(rendered_data[key][0], cv2.COLOR_BGRA2BGR))
            self.mycounter += 1
