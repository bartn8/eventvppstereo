...
class ExpLogger:
    ...
    def __init__(self, save_root, mode='train'):
        assert mode in ['train', 'validation', 'test', 'search']
    ...
    def colormap_image(self,x,vmax=None,cmap='magma'):
        ...
        return torch.from_numpy(np.transpose(colormapped_im,(2,0,1)))

    def add_image(self, tag, image, global_step):
        if self._tensor_log is None:
            self._tensor_log = SummaryWriter(self._save_root)
        self._tensor_log.add_image(tag=tag, img_tensor=self.colormap_image(image,cmap='magma'), global_step=global_step)
